...
<div class="flex flex-col border-white/10 border border-t-0 bg-zinc-800/10 h-52 overflow-y-scroll scrollbar scrollbar-thumb-zinc-600 scrollbar-none">
		{#each items as item, index}
			<button in:fade|global={{delay: index * 50}} class="flex items-center w-full min-h-14 relative border-b border-white/10 px-4 hover:bg-zinc-400/5 transition-colors overflow-clip" on:click={() => onItemSelect(index)}>
...
<div class="block text-sm text-zinc-400 w-full mt-2 ">
	{#each items.filter(item => item.selected) as item}
		<button class="flex justify-start items-center w-full h-8 bg-white/5 text-left px-3 border-b border-white/10 last:border-transparent">
